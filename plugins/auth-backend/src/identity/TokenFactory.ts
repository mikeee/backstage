...
const MS_IN_S = 1000;
const MAX_TOKEN_LENGTH = 8192; // A common total header limit for many HTTP servers
...
export class TokenFactory implements TokenIssuer {
  ...
  async issueToken(params: TokenParams): Promise<string> {
    ...
    if (token.length > MAX_TOKEN_LENGTH) {
      throw new Error(
        `Failed to issue a new user token. The resulting token is excessively large, with either too many ownership claims or too large custom claims. The following claims were requested: '${JSON.stringify(
          claims,
        )}'`,
      );
    }
